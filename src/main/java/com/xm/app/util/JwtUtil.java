...
public class JwtUtil {
   ...
   // private static Logger log = LoggerFactory.getLogger(UserServiceImpl.class);
    ...
    public static String createJWT(String id, String subject, long ttlMillis, Users users) throws Exception {
        ...
        JwtBuilder builder = Jwts.builder()
                ...
                .setClaims(claims)
                //这里是JWT的唯一标识，根据业务需要，这个可以设置为一个不重复的值，主要用来作为一次性token,从而回避重放攻击
                .setId(UUID.randomUUID().toString())
                //  jwt的签发时间
                .setIssuedAt(now)
                // 这里代表这个JWT的主体，可以是用户名，id，保证唯一就可以了
                .setSubject(subject)
                // 设置签名使用的签名算法和签名使用的秘钥
